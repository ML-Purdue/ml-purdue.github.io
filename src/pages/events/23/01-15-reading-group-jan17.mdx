---
layout: "../../../layout/base.astro"
title: "Reading Group - Jan 17"
description: We will have our first meeting after syllabus week on January 17th! Jinen will be presenting Chapter 2 "Groups, Rings, and Fields" from the textbook Algebra
---
import ContentSection from "~/components/content-section.astro";
import ListItem from "~/components/data/ListItem.astro";

<ContentSection title="Reading Group - Jan 17">

<div class='w-full'>
We will have our first meeting after syllabus week on January 17th! Jinen will be presenting Chapter 2 "Groups, Rings, and Fields" from the textbook Algebra, Topology, Differential Calculus, and Optimization Theory For Computer Science and Machine Learning.

Date: Tuesday, Jan 17

Time: 6:30 - 8:30 pm

Location: LWSN B134

</div>

</ContentSection>
