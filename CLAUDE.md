...
**Data-driven content.** Reading-group and workshop entries live in `src/data-entries.ts` as typed arrays (`RGItems`, `workshopItems`), with shapes defined in `src/types.ts` (`RGItem`, `WorkshopItem`). To add/edit one, edit these arrays rather than touching page markup. Pages import them and render through components in `src/components/`. When adding a field, update the interface in `src/types.ts` first.

**Projects** are a separate Astro content collection — one markdown file per project in `src/content/projects/`, with frontmatter validated by the Zod schema in `src/content/config.ts` and rendering helpers in `src/lib/projects.ts`. A project is either `status: active` (with an `intake` of `open`/`full`/`tbd`) or `status: past`; archiving is a two-line frontmatter edit, not a deletion. Terms are stored machine-readable (`2025-fall`) so they sort and group by academic year. `src/content/projects/README.md` is the full field reference and maintenance guide — read it before changing the schema. This content was migrated off Notion in August 2026; the site is now the sole source of truth for project info, so don't reintroduce links to the old Notion database.

**Pages** (`src/pages/*.astro`) map 1:1 to routes (`sponsor.astro` → `/sponsor`), except projects, which uses `src/pages/projects/` (`index.astro`, `archive.astro`, and the dynamic `[slug].astro` detail page). Navigation is defined as the `navItems` array in `src/components/header.astro` — add a route there to surface it in the nav. Note: most pages currently re-declare their own full `<html>`/`<head>` boilerplate (title, OG/Twitter meta) inline instead of using `src/layout/base.astro`. When editing head/meta, expect to change it per-page.
